...
if not Path(db_path).is_file():
    ...
    cur.execute("""CREATE TABLE printed (
    ...
    def api(self, type=""):
        ...
        def api_sheets():
            ...
            for match in matches:
                outputs = {"team1": {"match": match["number"], "reverse_stage": max_stage - match["stage"], "team": ""}, "team2": {"match": match["number"], "reverse_stage": max_stage - match["stage"], "team": ""}}
            
                for key in ["team1", "team2"]:
                    if match[key][:1] == "w":
                        source_match = cur.execute("SELECT match_number FROM match_structure WHERE id=? LIMIT 1", (match[key][1:],)).fetchall()[0][0]
                        if source_match in winners:
                            outputs[key]["team"] = str(winners[source_match])
                        else:
                            outputs[key]["team"] = ""
                    else:
                        outputs[key]["team"] = str(match[key])
                
                if outputs["team1"]["team"] != "":
                    sheets.append(outputs["team1"])
                if outputs["team2"]["team"] != "":
                    sheets.append(outputs["team2"])
    
            #Check if printed
            for i in range(len(sheets)):
                result = cur.execute("SELECT COUNT() FROM printed WHERE match=? AND team=?", (sheets[i]["match"], sheets[i]["team"])).fetchall()[0][0]
                if result > 0:
                    sheets[i]["printed"] = True
                else:
                    sheets[i]["printed"] = False
            conn.close()
...
<head>
    <title>
        Printing - FLL Playoff Display
    </title>
<link rel="stylesheet" type="text/css" href="/static/css/printing.css"></link>
</head>
<body>
    <div id="sheetList">
    Loading sheet list...
